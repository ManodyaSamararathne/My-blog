...
async function createPost(event, baseUrl) {
   ...
   const headers = new Headers({
    ...
    Authrization: `Bearer ${storedToken}`,
   });
...
async function registerUser(event, baseUrl) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const baseUrl = window.location.origin;
        const registerDiv = document.getElementsById('register-div');
        const loginDiv = document.getElementsById('login-div');
        const logoutDiv = document.getElementsById('logout-div');
        const logoutButton = document.getElementsById('logout-button');
